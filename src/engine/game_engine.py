        ...
        while not self.state.is_game_over():
            ...
            if self.state.cards_exhausted:
                ...
                if self.state.round != -1:
                    ...
                    if EXPANSION: self.state.extend_base_phase()

            self.state.start_new_round()

            for player in turn_order:
                player.cards.append(sample(self.state.map.available_tiles, NUM_CARDS_DRAWN_PER_ROUND))
                
                # wait for player to place card
                placeholder_tile_placed: Tile = Tile.get_starting_tile()

                completed_components = self.state.check_any_complete(placeholder_tile_placed)

                for edge in completed_components:

                    reward = self.state._get_reward(placeholder_tile_placed, edge)

                    for player_id in self.state._get_claims(placeholder_tile_placed, edge):
