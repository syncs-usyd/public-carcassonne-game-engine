...
class BotState:
    ...
    def __init__(self):
        ...
        self.strategy_preference = "balanced"  # Can be "aggressive", "defensive", "balanced"
        self.target_structures: List[str] = []  # Structures we're focusing on completing
    ...
    def update_strategy(self, game_state):
        ...
        opponent_points = [p.points for p in game_state.players.values() if p.player_id != game_state.me.player_id]
...
def handle_place_tile(game: Game, bot_state: BotState, query: QueryPlaceTile) -> MovePlaceTile:
    ...
    best_score, best_tile_idx, best_tile, best_x, best_y, best_rotation = scored_placements[0]
    ...
    print(f"Claude placing tile {best_tile.tile_type} at ({best_x}, {best_y}) with rotation {best_rotation}, score: {best_score}")
...
def handle_place_meeple(game: Game, bot_state: BotState, query: QueryPlaceMeeple) -> MovePlaceMeeple | MovePlaceMeeplePass:
    ...
    try:
        ...
        unclaimed_structures = get_unclaimed_structures(game, bot_state.last_tile, structures)
        ...
        if best_score > 3 or (best_score > 1 and bot_state.strategy_preference == "aggressive"):
            bot_state.target_structures.append(f"{bot_state.last_tile.pos}_{best_edge}")
            bot_state.last_tile = None
            print(f"Claude placing meeple on {best_edge} (structure: {best_structure}, score: {best_score})")
            return game.move_place_meeple(query, bot_state.last_tile, placed_on=best_edge)
                    ...
                    for tile_idx, tile in enumerate(game.state.my_cards):
                        ...
                        for rotation in range(4):
                            ...
                            if game.can_place_tile_at(tile, adj_x, adj_y):
                                placements.append((tile_idx, tile, adj_x, adj_y, rotation))
                        tile.rotation = original_rotation  # Reset rotation
    
    return placements


def evaluate_tile_placement(game: Game, bot_state: BotState, tile, x: int, y: int, rotation: int) -> float:
    """
...
def evaluate_opponent_blocking(game: Game, tile, x: int, y: int) -> float:
    ...
    opponent_meeples = [m for m in all_meeples if m.player_id != game.state.me.player_id]
...
def evaluate_future_potential(game: Game, tile, x: int, y: int) -> float:
    ...
    for dx, dy in [(0, 1), (1, 0), (0, -1), (-1, 0)]:
        ...
        if (0 <= adj_x < width and 0 <= adj_y < height and 
            grid[adj_y][adj_x] is None):
            open_edges += 1
    
    return open_edges * 0.3


def evaluate_meeple_placement(game: Game, bot_state: BotState, edge: str, structure_type: StructureType) -> float:
    """
    ...
    if bot_state.strategy_preference == "aggressive":
        score += 1.0
    elif bot_state.strategy_preference == "defensive" and structure_type == StructureType.CITY:
        score += 2.0
...
def fallback_place_tile(game: Game, bot_state: BotState, query: QueryPlaceTile) -> MovePlaceTile:
...
def get_unclaimed_structures(game: Game, tile_model: TileModel, structures: Dict[str, StructureType]) -> Dict[str, StructureType]:
    """
        ...
        if edge in adjacent_positions:
            ...
            if (0 <= adj_y < len(grid) and 0 <= adj_x < len(grid[0])):
                ...
                if adjacent_tile is not None:
                    ...
                    if (hasattr(tile, 'internal_edges') and hasattr(adjacent_tile, 'internal_edges')):
                        if (edge in tile.internal_edges and 
                            opposite_edge in adjacent_tile.internal_edges and
                            tile.internal_edges[edge] == adjacent_tile.internal_edges[opposite_edge]):
                            
                            # Structures match, check if adjacent structure is claimed
                            if (hasattr(adjacent_tile, 'internal_claims') and 
                                opposite_edge in adjacent_tile.internal_claims and
                                adjacent_tile.internal_claims[opposite_edge] is not None):
